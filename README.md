...
- **First-boot setup wizard** — a 5-step welcome overlay (`src/components/WelcomeWizard.jsx`) introduces the streamer on first launch (welcome → **connect your music** → **shape your sound** → phone control → all set). The connect step runs the real account flows inline — Spotify (OAuth), Tidal (device-code) and Qobuz (email/password) — all optional. The sound step applies a DSP/EQ mode in one tap (Manual EQ / Room Correction / Pure Direct). Completion is stored in SQLite (`onboarding_complete`) so it only appears once; re-runnable any time from **Settings → Run Setup Wizard**, and it reappears automatically after a factory reset
...
- **Universal search** — one query fans out to every connected source at once (Spotify, local library, web radio, Tidal, Qobuz — whichever are actually connected) and results come back grouped by source, not limited to whatever's currently playing; an optional filter chip narrows the same result set down to one source afterward
