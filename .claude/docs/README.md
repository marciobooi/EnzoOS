...
|---|---|---|
| [camilladsp.md](camilladsp.md) | Config schema, WebSocket API (full command list), filter types (Biquad sub-types, Conv/FIR), dither, rate adjust | CamillaDSP 4.1.3 (pinned in install.sh) |
...
| [mpd.md](mpd.md) | Resampler (soxr), ALSA output opts, outputs (incl. Digital Transport), protocol (lsinfo/readpicture/albumart/include_optional), the `-f`/`\|` format-string gotcha | MPD 0.23.14 (unpinned — genuinely drifts between 0.23.5/0.23.14 with apt updates; verify live) |
