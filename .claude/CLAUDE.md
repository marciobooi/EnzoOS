...
- PW loopback bridge verified working 2026-08-25: played `pw-play --target=ResonanceInput` test noise and polled CamillaDSP's `GetCaptureSignalPeak` over WS — signal registered at -18 to -22 dB during playback, -1000 (silence) between loops. The 2026-07-02 "known-broken/target.object mismatch" note (TODO §9.1) no longer reproduces; both the MPD path and the PipeWire-sources path (raspotify/shairport/BT/Chromium → ResonanceInput) reach CamillaDSP.
- CamillaDSP config is regenerated by `server/camilla-config.js:updateCamillaConfigFromSettings()` on every server start — changes to camilladsp.yml are overwritten
...
- `server/camilla-config.js` — CamillaDSP YAML config generator (`generateCamillaConfig()` — moved out of player.js), `ensureAsoundConf()`, EQ/FIR/Pure Direct/headroom logic — see `.claude/docs/camilladsp.md`
- `server/mpd-art.js` — embedded cover art via MPD's `readpicture`/`albumart` binary protocol
- `server/fir-filters.js` — FIR/convolution filter upload (WAV impulse response import)
- `server/mpd-transport.js` — Digital Transport (generalizes DSD Native Bypass into a user-picked ALSA output, bypassing CamillaDSP)
...
- `/etc/mpd.conf` — MPD config (written by install.sh); `include_optional`s `/etc/mpd-digital-transport.conf` (written by `server/mpd-transport.js` at runtime, not install time — the device target is user-chosen)
