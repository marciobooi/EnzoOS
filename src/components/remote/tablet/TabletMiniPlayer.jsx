...
export default function TabletMiniPlayer() {
  ...
  return (
    <div className="relative">
      ...
      {canSkip && (
        <div className="absolute left-6 right-6 -bottom-[7px] h-[14px] rounded-b-[10px] overflow-hidden"
      ...
      <div className="relative rounded-[24px] flex items-center gap-4 px-4 py-3.5 cursor-pointer active:scale-[0.99] transition-transform"
        style={{
        ...
        <div className="w-16 h-16 rounded-[14px] overflow-hidden flex items-center justify-center shrink-0"
          ...
          {albumImage
            ...
            : <Music className="h-6 w-6" style={{ color: C.text4 }} />}
        </div>

        <div className="flex-1 min-w-0">
          <p className="text-[19px] font-semibold truncate" style={{ color: C.text1, letterSpacing: '-0.01em' }}>
            {trackName}
          </p>
          <p className="text-[12px] font-semibold uppercase truncate mt-0.5"
        ...
        <div className="flex items-center gap-2 shrink-0 pr-1">
          {canSkip && (
            <button onClick={e => { e.stopPropagation(); handlePrevious(); }} disabled={skipDisabled}
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full active:scale-90 transition-all disabled:opacity-25 cursor-pointer">
              <SkipBack className="h-5 w-5" strokeWidth={1.75} style={{ color: C.text1 }} />
            </button>
          )}
          <button
            ...
            className="w-14 h-14 rounded-full flex items-center justify-center shrink-0 active:scale-95 transition-all disabled:opacity-25 cursor-pointer"
            style={{ background: accentFill }}>
            {isPlaying
              ? <Pause className="h-5 w-5" style={{ fill: accentFillHex, color: accentFillHex }} />
              : <Play  className="h-5 w-5 ml-0.5" style={{ fill: accentFillHex, color: accentFillHex }} />}
          </button>
          {canSkip && (
            <button onClick={e => { e.stopPropagation(); handleNext(); }} disabled={skipDisabled}
              aria-label={t('player.next')}
              className="w-10 h-10 flex items-center justify-center rounded-full active:scale-90 transition-all disabled:opacity-25 cursor-pointer">
              <SkipForward className="h-5 w-5" strokeWidth={1.75} style={{ color: C.text1 }} />
            </button>
          )}
        </div>
      </div>
