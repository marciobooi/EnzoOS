            ...
            <div className="space-y-1">
              {EQ_PRESETS.map((p) => (
                <button
                  ...
                  className={`w-full text-left font-mono text-[10px] px-3 py-1.5 rounded-lg cursor-pointer transition-all border ${
                    currentPreset === p.name
                      ? 'bg-[var(--theme-color-dim)] border-[var(--theme-color)]/50 text-[var(--theme-color)] shadow-[0_0_8px_var(--theme-color-glow)]'
                      : 'bg-zinc-900/40 hover:bg-zinc-800/60 border-white/5 text-zinc-300'
                  }`}
                >
          ...
          <div className="flex justify-around items-end h-[110px] pb-1 relative">
            ...
            {bands.map((bandVal, index) => (
              <div key={index} className="flex flex-col items-center h-full relative z-10 w-12 justify-end">
                ...
                {/* Visual groove for path slider */}
                <div className="relative w-1.5 bg-zinc-800 h-[65px] rounded-full flex items-center justify-center">
                  <input
                    type="range"
                    min="-12"
                    max="12"
                    ...
                    className="absolute inset-0 opacity-0 cursor-row-resize h-full w-full"
                  />
                  {/* Styled slider track */}
                  <div 
                  ...
                  {/* Styled slider knob */}
                  <div 
                    className="absolute w-3.5 h-3.5 rounded-full bg-white border-2 border-[var(--theme-color)] shadow-[0_0_6px_var(--theme-color-glow)] transition-shadow hover:scale-110"
