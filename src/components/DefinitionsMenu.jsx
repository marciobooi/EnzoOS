...
import { Sliders, RefreshCw, Speaker, Laptop, Check, AlertCircle, Music, Download, LogOut, Disc, RotateCw } from 'lucide-react';
import { api } from '../api';
  ...
  return (
    <div className="flex flex-row gap-6 font-sans text-zinc-800 h-full pb-3 pr-4 items-stretch select-none">
      ...
      <button
        ...
        className={`w-[180px] shrink-0 p-5 rounded-2xl border text-left flex flex-col justify-between transition-all duration-300 relative group overflow-hidden cursor-pointer shadow-[0_8px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_12px_28px_rgba(0,0,0,0.22)] ${
          spotify 
            ? 'border-2 border-[var(--theme-color)] bg-gradient-to-b from-white to-zinc-50 scale-[1.02]' 
      ...
      >
        ...
        <div className="my-auto flex justify-center py-2">
          <svg 
            ...
            className={`h-16 w-16 transition-all duration-300 ${
              spotify ? 'fill-[#1ed760] drop-shadow-[0_0_15px_rgba(30,215,96,0.35)]' : 'fill-zinc-400 group-hover:fill-zinc-500'
            }`}
          >
        ...
        <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-wider w-full">
          <span className={spotify ? 'text-[#1ed760] font-extrabold' : 'text-zinc-500'}>SPOTIFY</span>
          {spotify && <span className="theme-text font-black">ACTIVE</span>}
        </div>
      </button>
      ...
      >
        ...
        <div className="my-auto flex justify-center py-2">
          <Music 
            className={`h-16 w-16 transition-all duration-300 ${
              !spotify ? 'text-amber-500 drop-shadow-[0_0_15px_rgba(245,158,11,0.35)]' : 'text-zinc-400 group-hover:text-zinc-500'
            }`}
          />
        </div>

        <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-wider w-full">
          <span className={!spotify ? 'theme-text font-extrabold' : 'text-zinc-500'}>LOCAL PLAYER</span>
          {!spotify && <span className="theme-text font-black">ACTIVE</span>}
        </div>
      </button>
      ...
      <button
        ...
        className="w-[180px] shrink-0 p-5 rounded-2xl border text-left flex flex-col justify-between transition-all duration-300 relative group overflow-hidden cursor-pointer shadow-[0_8px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_12px_28px_rgba(0,0,0,0.22)] border-zinc-200/80 bg-gradient-to-b from-[#ffffff] to-[#eef2f7] hover:border-zinc-300 hover:scale-[1.01]"
      >
        <span className="text-[9px] font-extrabold tracking-widest text-zinc-400 uppercase">APPEARANCE</span>
        
        <div className="my-auto flex justify-center py-2">
          <Sliders 
            className="h-16 w-16 text-zinc-400 group-hover:text-zinc-500 transition-colors"
          />
        </div>

        <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-wider w-full">
          <span className="text-zinc-500">THEME</span>
          <span className="theme-text font-extrabold">{theme?.toUpperCase()}</span>
        </div>
      </button>
            ...
            <Download 
              className={`h-16 w-16 transition-all duration-300 ${
                updateStatus === 'available' 
                  ? 'text-amber-500 drop-shadow-[0_0_15px_rgba(245,158,11,0.35)] animate-bounce' 
      ...
      {token && (
        <button
          ...
          className="w-[180px] shrink-0 p-5 rounded-2xl border text-left flex flex-col justify-between transition-all duration-300 relative group overflow-hidden cursor-pointer shadow-[0_8px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_12px_28px_rgba(0,0,0,0.22)] border-zinc-200/80 bg-gradient-to-b from-[#ffffff] to-[#eef2f7] hover:border-rose-300 hover:scale-[1.01]"
        >
          <span className="text-[9px] font-extrabold tracking-widest text-zinc-400 uppercase">CONNECTIONS</span>
          
          <div className="my-auto flex justify-center py-2">
            <LogOut 
              className="h-16 w-16 text-zinc-400 group-hover:text-rose-500 transition-colors"
            />
          </div>
