...
// Defined outside TrackSearch so React sees a stable type reference across
// renders — prevents every list item from unmounting/remounting on state change.
    ...
    <div className="shrink-0 w-[84px] select-none" style={{ scrollSnapAlign: 'start' }}>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center hover:bg-black/30 transition-colors">
          <div className="opacity-0 hover:opacity-100 transition-opacity p-1.5 rounded-full bg-white/90">
            <Play className="h-4 w-4 fill-current" style={{ color: S.accent }} />
          </div>
        </div>
      </button>
      <p className="text-[9px] font-bold mt-1.5 leading-tight line-clamp-2 select-none" style={{ color: S.strong }}>{title}</p>
...
function TrackRow({ track, onPlay }) {
  ...
  return (
    ...
    >
      {art
        ? <img src={art} alt={track.name} className="w-8 h-8 rounded-lg shrink-0 object-cover" loading="lazy" />
        : <div className="w-8 h-8 rounded-lg shrink-0 flex items-center justify-center" style={{ background: S.surfaceLo }}>
            <Music className="h-3.5 w-3.5" style={{ color: S.muted }} />
          </div>
      }
...
// Drawer sub-components (remote control layout) — also module-level
function DrawerTrackRow({ track, onPlay }) {
  if (!track) return null;
  const art = track.album?.images?.[2]?.url || track.album?.images?.[0]?.url;
  return (
    <div className="p-2.5 rounded-xl flex items-center gap-3 border border-zinc-100 bg-white hover:border-zinc-300 hover:shadow-sm transition-all">
      {art
        ? <img src={art} alt={track.name} className="w-9 h-9 rounded shrink-0 object-cover" loading="lazy" />
        : <div className="w-9 h-9 rounded shrink-0 bg-zinc-100 flex items-center justify-center"><Music className="h-4 w-4 text-zinc-400" /></div>
      }
      <div className="min-w-0 flex-grow">
      ...
      <button onClick={() => onPlay(track.uri)} className="p-2 rounded-lg bg-zinc-900 text-white shrink-0 active:scale-95 cursor-pointer">
...
function DrawerContextCard({ item, type, onClick }) {
  ...
  return (
    <button onClick={() => onClick(item)} className="flex items-center gap-3 p-2.5 rounded-xl border border-zinc-100 bg-white hover:border-zinc-300 hover:shadow-sm transition-all w-full text-left cursor-pointer">
      {image
        ? <img src={image} className="w-11 h-11 rounded-lg shrink-0 object-cover" alt={item.name} loading="lazy" />
        : <div className="w-11 h-11 rounded-lg shrink-0 bg-zinc-100 flex items-center justify-center">
            {type === 'playlist' ? <ListMusic className="h-5 w-5 text-zinc-400" /> : <Disc3 className="h-5 w-5 text-zinc-400" />}
          </div>
      }
      <div className="min-w-0 flex-grow">
        <p className="text-xs font-bold truncate text-zinc-800">{item.name}</p>
        <p className="text-[9px] truncate text-zinc-500">
          {type === 'playlist' ? `${item.tracks?.total || 0} tracks` : item.artists?.map(a => a.name).join(', ')}
        </p>
      </div>
    </button>
  );
...
export default function TrackSearch({ token, onPlayTrack, onPlayContext, isDrawer = false }) {
  ...
  const handleDrilldownPlaylist = useCallback(async (pl) => {
    setIsLoading(true);
    try {
  ...
  const handleDrilldownAlbum = useCallback(async (album) => {
    setIsLoading(true);
    try {
    ...
    return (
      <div className="flex flex-col h-full font-mono relative">
        <div className="flex items-center justify-between mb-4 pb-2.5 border-b border-zinc-200">
          {view !== 'browse'
            ? <button onClick={handleBack} className="flex items-center gap-1 text-[10px] font-bold uppercase text-zinc-500 hover:text-zinc-800 cursor-pointer"><ChevronLeft className="h-3.5 w-3.5" />Back</button>
            : <h3 className="text-xs font-bold tracking-widest uppercase flex items-center gap-2 text-zinc-800"><Search className="h-4 w-4" />Spotify</h3>
          }
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-4">
          <input ref={inputRef} type="text" placeholder="Search songs, albums, playlists…" value={query}
            onChange={e => { setQuery(e.target.value); if (!e.target.value.trim()) { setView('browse'); setResults(null); } }}
            onKeyDown={e => e.key === 'Enter' && handleSearch(e)}
            className="flex-grow py-2.5 px-3 rounded-xl text-xs focus:outline-none bg-zinc-50 border border-zinc-200 text-zinc-800 placeholder-zinc-400" />
          <button type="submit" disabled={isLoading} className="py-2.5 px-4 rounded-xl text-xs border bg-zinc-900 text-white border-zinc-900 cursor-pointer active:scale-95">
        ...
        <div className="flex-grow overflow-y-auto max-h-[80vh]">
          {view === 'browse' && (isBrowseLoading
            ? spinner
            : <div className="space-y-2">
                {recentTracks.slice(0,6).map((t,i) => <DrawerTrackRow key={`r${t?.uri}${i}`} track={t} onPlay={onPlayTrack} />)}
                {playlists.map(pl => <DrawerContextCard key={pl.id} item={pl} type="playlist" onClick={handleDrilldownPlaylist} />)}
              </div>
          )}
          {view === 'search' && results && (
            <div className="space-y-2">
              {results.tracks?.items?.map(t => <DrawerTrackRow key={t.id} track={t} onPlay={onPlayTrack} />)}
              {results.albums?.items?.map(a => <DrawerContextCard key={a.id} item={a} type="album" onClick={handleDrilldownAlbum} />)}
              {results.playlists?.items?.filter(Boolean).map(pl => <DrawerContextCard key={pl.id} item={pl} type="playlist" onClick={handleDrilldownPlaylist} />)}
            </div>
          )}
          {(view === 'playlist' || view === 'album') && (
            <div className="space-y-2">
              {drilldownTracks.map((t,i) => <DrawerTrackRow key={`d${t?.uri}${i}`} track={t} onPlay={onPlayTrack} />)}
            </div>
          )}
        </div>
      </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 flex-grow">
          <div className="relative flex-grow">
            <input
              ...
              onKeyDown={e => e.key === 'Enter' && handleSearch(e)}
              className="w-full py-2 pl-3 pr-8 rounded-xl text-[11px] font-semibold focus:outline-none tracking-wide"
              style={{ background: S.surfaceLo, border: `1px solid ${S.border}`, color: S.strong }}
            />
            {query && (
      ...
      <div className="flex-grow min-h-0 overflow-y-auto space-y-4">
        ...
        {view === 'browse' && (isBrowseLoading ? spinner : (
          <>
                ...
                <HScroll>
                  {playlists.map(pl => (
                    <ArtCard key={pl.id}
                      ...
                      onAction={() => handleDrilldownPlaylist(pl)}
                    />
                  ))}
                </HScroll>
              </Rail>
            ...
            {!isBrowseLoading && !recentTracks.length && !topTracks.length && !playlists.length && (
              ...
              <Rail icon={Disc3} label="Albums" count={results.albums.items.length}>
                <HScroll>
                  {results.albums.items.map(album => (
                    <ArtCard key={album.id}
                      image={album.images?.[0]?.url}
                      title={album.name}
                      subtitle={album.artists?.map(a => a.name).join(', ')}
                      onAction={() => handleDrilldownAlbum(album)}
                    />
                  ))}
                </HScroll>
              </Rail>
        ...
        {(view === 'playlist' || view === 'album') && (
          <>
            <div className="flex items-center gap-3 p-2.5 rounded-2xl shrink-0" style={{ background: S.surfaceLo, border: `1px solid ${S.border}` }}>
              {drilldownImage
              ...
              <div className="min-w-0 flex-grow">
                <p className="text-sm font-extrabold truncate" style={{ color: S.strong }}>{drilldownTitle}</p>
                <p className="text-[10px] mt-0.5" style={{ color: S.muted }}>{drilldownTracks.length} tracks</p>
              </div>
              {onPlayContext && drilldownUri && (
                <button onClick={() => onPlayContext(drilldownUri)}
            ...
            {isLoading ? spinner : (
              <div>
                {drilldownTracks.map((track, i) => (
                  <TrackRow key={`drill-${track?.uri || i}`} track={track} onPlay={onPlayTrack} />
                ))}
                {!drilldownTracks.length && (
