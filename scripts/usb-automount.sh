...
# ownership so MPD can actually read the files — then wires the mount into
# MPD's virtual filesystem via the "local" storage plugin's mount/unmount
# commands (MPD 0.23+), which makes it instantly browsable with no restart
# and no change to music_directory. Finally pings the running server so any
# open remote/kiosk gets a live "USB connected" update.
#
# Usage: usb-automount.sh add|remove /dev/sdXN
# Runs as root (udev RUN+=); the mount/unmount calls are dropped to pi via `su`
# since udisksctl talks to the system D-Bus and doesn't need a full session.
set -uo pipefail
...
case "$ACTION" in
  add)
    ...
    su -s /bin/bash pi -c "mpc mount usb '$MOUNT_PATH'" 2>&1 | logger -t resonance-usb
  ...
  remove)
    ...
    su -s /bin/bash pi -c "mpc unmount usb" 2>&1 | logger -t resonance-usb
